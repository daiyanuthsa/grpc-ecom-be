                ...
                script {
                    ...
                    withCredentials([
                        ...
                        stage('Remote: Docker Login') {
                            sh """
                                ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i \$SSH_KEY \$SSH_USER@${vmIp} "DOCKER_USER='${DOCKER_USER}' DOCKER_PASS='${DOCKER_PASS}' <<'EOF'
                                set -e
                        ...
                        stage('Remote: Build gRPC Backend') {
                            sh """
                                ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i \$SSH_KEY \$SSH_USER@${vmIp} "DOCKER_USER='${DOCKER_USER}' <<'EOF'
                                set -e
                                cd grpc-ecom-be
